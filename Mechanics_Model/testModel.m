function [anchor_detach_count_total, distal_attach_count_total, total_anchor_adhered , total_distal_adhered, total_breakage, total_length] = testModel(flowRate, initialCells)
...
for i = 1:1000000 
    ...
    if totalcells(1) >1  %if we are only 1 cell, we don't check if the cells break 
        ...
        [cell_pos_break, anchor_num, distal_adhered, breakage] = checkBreakage(cell_pos_detach, tension);
...
%Plot chains. Only do for single chain at a time (ie not
%runMulipleChains scripts)

%     if mod(i,100) == 0
%         %hold on
%         plot(cell_pos(:,1), cell_pos(:,2), 'ro-', 'LineWidth', 2)
%         ylim([-0.1,6]); 
%         xlim([0,6.5]);
%         %title('Low Flow')%- with Kick, Growth, Adherence, Detachment, and Breakage'])
%         %axis square
%         f(end+1) = getframe;

    end

%write to a video. Only do for single chain at a time (ie not
%runMulipleChains scripts)
% 
% writerObj = VideoWriter('LowFlow_Talk_updatedColors.avi');
% open(writerObj);
